...
class AdminSalaryViewModel extends ChangeNotifier {
  ...
  List<SalaryPeriodSummary> get salaryPeriods => List.unmodifiable(_salaryPeriods);
  ...
  Future<SalaryPeriodDetailData?> getSalaryPeriodDetail(int salaryPeriodId) {
    return _employeeRepository.getSalaryPeriodDetail(_companyId, salaryPeriodId);
  }
    ...
    try {
      ...
      if (response?.responseStatus == true) {
        final successMessage =
            ...
            'Salary period approved successfully';
        await refresh();
        _message = successMessage;
        return true;
      }
  ...
  Future<bool> markSalaryPaid({
    required int salaryPeriodId,
    String? paymentRef,
  }) async {
    if (_companyId <= 0) return false;
    ...
    try {
      final response = await _employeeRepository.markSalaryPeriodPaid(
        _companyId,
        salaryPeriodId,
        request: MarkSalaryPaidRequest(paymentRef: paymentRef),
      );
      if (response?.responseStatus == true) {
        final successMessage =
            response?.responseMessage ?? 'Salary marked as paid successfully';
        await refresh();
        _message = successMessage;
        return true;
      }
      _error = response?.responseMessage ?? 'Failed to mark salary as paid';
      return false;
    } catch (e) {
      _error = 'Failed to mark salary as paid';
      return false;
    } finally {
      _isSaving = false;
      notifyListeners();
